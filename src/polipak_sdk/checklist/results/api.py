...
class ResultsApi(BaseApi):
    ...
    def get(self, result_id: int):
        return self._client.request('GET', path=f'{self.BASE_PATH}/{result_id}/')

    def create(self, data: dict):
        return self._client.request('POST', path=f'{self.BASE_PATH}/', json=data)

    def update(self, result_id: int, data: dict):
        return self._client.request('PUT', path=f'{self.BASE_PATH}/{result_id}/', json=data)

    def delete(self, result_id: int):
        return self._client.request('DELETE', path=f'{self.BASE_PATH}/{result_id}/')

    def history(self, result_id: int):
        return self._client.request('GET', path=f'{self.BASE_PATH}/{result_id}/history/')
